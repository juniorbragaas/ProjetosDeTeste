...
namespace TGTCE.Repository.Implementation
{
    public class FaturaRepository : IFaturaRepository
    {
        ...
        public FaturaRepository(BaseContext _db)
        {
            db = _db;
        }
        ...
        public List<Fatura> BuscarPorData(string dataInicial, string dataFinal)
        {
            if (db != null)
            {
                return db.Fatura.Where(e => e.dataEnvio >= Convert.ToDateTime(dataInicial) && e.dataEnvio <= Convert.ToDateTime(dataFinal)).ToList(); 
            }
